...
if [ -f "$ENV_FILE" ]; then
  ...
  TRAINER_REGISTRATION_ENABLED=$(sudo grep '^TRAINER_REGISTRATION_ENABLED=' "$ENV_FILE" | cut -d= -f2- | tr -d '\r' || true)
  TRAINER_MAX_USERS=$(sudo grep '^TRAINER_MAX_USERS=' "$ENV_FILE" | cut -d= -f2- | tr -d '\r' || true)
  TRAINER_INACTIVE_ACCOUNT_DAYS=$(sudo grep '^TRAINER_INACTIVE_ACCOUNT_DAYS=' "$ENV_FILE" | cut -d= -f2- | tr -d '\r' || true)
else
...
export TRAINER_REGISTRATION_ENABLED="${TRAINER_REGISTRATION_ENABLED:-true}"
export TRAINER_MAX_USERS="${TRAINER_MAX_USERS:-500}"
export TRAINER_INACTIVE_ACCOUNT_DAYS="${TRAINER_INACTIVE_ACCOUNT_DAYS:-365}"
...
DEPLOY_ENV=(
  ...
  "TRAINER_REGISTRATION_ENABLED=$TRAINER_REGISTRATION_ENABLED"
  "TRAINER_MAX_USERS=$TRAINER_MAX_USERS"
  "TRAINER_INACTIVE_ACCOUNT_DAYS=$TRAINER_INACTIVE_ACCOUNT_DAYS"
)
